...
func carveTAR(data []byte, offset int) []byte {
	...
	for s.hasMore() {
		...
		if !equalBytes(s.getBytes(5), []byte("ustar")) {
...
func carveBZIP2(data []byte, offset int) []byte {
	...
	for _, marker := range bzip2EndOfStream {
		...
		if equalBytes(s.getBytes(len(marker)), marker) {
			break
		}
...
func carveDEB(data []byte, offset int) []byte {
	...
	for s.pos+arHeaderWidth <= s.length() {
		if !equalBytes(s.bytes[s.pos+arMagicOffset:s.pos+arHeaderWidth], arHeaderMagic) {
			break
		}
...
// equalBytes reports whether two byte slices hold the same bytes.
