package ops

// Pure-Go port of CyberChef's GOST hash engine
// (../CyberChef/src/core/vendor/gost/gostDigest.mjs, by Rudolf Nickolaev,
// derived from Alexey Degtyarev's streebog.net implementation). It covers the
// two digests the GOST Hash operation exposes: GOST R 34.11-2012 "Streebog"
// (256/512-bit) and the older GOST R 34.11-94 (based on the GOST 28147-89
// block cipher). The Streebog C and Ax tables are carried verbatim as the
// base64 blobs from the upstream source and decoded at init.

import (
	"encoding/hex"

	"github.com/roberson-io/cchef/core"
)

// gostStreebogCB64 / gostStreebogAxB64 are the Streebog constant tables C
// (12 x 512-bit) and Ax (precalculated LPS lookup), base64-encoded exactly as
// in the upstream engine.
const gostStreebogCB64 = ``

const gostStreebogAxB64 = ``

func init() {
	core.Register(GOSTHash{})
	decodeStreebogTables()
}

// gostB64decode mirrors the upstream base64 decoder used to
// unpack the Streebog constant tables (standard base64 alphabet, no padding).
func gostB64decode(s string) []byte {
	n := len(s)
	k := (n*3 + 1) >> 2
	r := make([]byte, k)
	var u24 uint32
	j := 0
	for i := range n {
		m4 := i & 3
		c := int(s[i])
		switch {
		case c > 64 && c < 91:
			c -= 65
		case c > 96 && c < 123:
			c -= 71
		case c > 47 && c < 58:
			c += 4
		case c == 43:
			c = 62
		case c == 47:
			c = 63
		default:
			c = 0
		}
		u24 |= uint32(c) << (18 - 6*m4)
		if m4 == 3 || n-i == 1 {
			for m3 := 0; m3 < 3 && j < k; m3, j = m3+1, j+1 {
				r[j] = byte(u24 >> ((16 >> m3) & 24) & 255)
			}
			u24 = 0
		}
	}
	return r
}

// Streebog constant tables, decoded at init from the base64 blobs above.
var (
	gostStreebogC  [12][]uint32 // twelve 512-bit round constants
	gostStreebogAx []uint32     // precalculated LPS lookup (4096 words)
)

func decodeStreebogTables() {
	cb := gostB64decode(gostStreebogCB64)
	for i := range 12 {
		blk := make([]uint32, 16)
		for j := range 16 {
			blk[j] = le32(cb, i*64+j*4)
		}
		gostStreebogC[i] = blk
	}
	ab := gostB64decode(gostStreebogAxB64)
	gostStreebogAx = make([]uint32, len(ab)/4)
	for i := range gostStreebogAx {
		gostStreebogAx[i] = le32(ab, i*4)
	}
}

// ---------------------------------------------------------------------------
// GOST R 34.11-2012 "Streebog".
// ---------------------------------------------------------------------------

// streebog holds the scratch buffers for one digest computation.
type streebog struct {
	sigma, n, h        []uint32
	data, ki, r, block []uint32
	buffer0, buffer512 []uint32
}

func newStreebog(bitLength int) *streebog {
	s := &streebog{
		sigma: make([]uint32, 16), n: make([]uint32, 16), h: make([]uint32, 16),
		data: make([]uint32, 16), ki: make([]uint32, 16), r: make([]uint32, 16),
		block: make([]uint32, 16), buffer0: make([]uint32, 16), buffer512: make([]uint32, 16),
	}
	s.buffer512[0] = 512
	if bitLength == 256 {
		for i := range s.h {
			s.h[i] = 0x01010101
		}
	}
	return s
}

func streebogGet8(x []uint32, i int) uint32 {
	return (x[i>>2] >> uint((i&3)*8)) & 0xff
}

func streebogAdd512(x, y []uint32) {
	var cf uint32
	for i := range 16 {
		w0 := (x[i] & 0xffff) + (y[i] & 0xffff) + cf
		w1 := (x[i] >> 16) + (y[i] >> 16) + (w0 >> 16)
		x[i] = (w0 & 0xffff) | (w1 << 16)
		cf = w1 >> 16
	}
}

func streebogXor512(x, y []uint32) {
	for i := range 16 {
		x[i] ^= y[i]
	}
}

// xlps applies the X (xor key), L (linear), P (permute), S (substitute)
// transform: x = LPS(x ^ y), using the precomputed Ax table.
func (s *streebog) xlps(x, y []uint32) {
	copy(s.r, x)
	streebogXor512(s.r, y)
	for i := range 8 {
		k := int(streebogGet8(s.r, i)) << 1
		z0, z1 := gostStreebogAx[k], gostStreebogAx[k+1]
		for j := 1; j < 8; j++ {
			k = (j << 9) + (int(streebogGet8(s.r, (j<<3)+i)) << 1)
			z0 ^= gostStreebogAx[k]
			z1 ^= gostStreebogAx[k+1]
		}
		x[i<<1] = z0
		x[(i<<1)+1] = z1
	}
}

// g is the Streebog compression function g_N(h, m).
func (s *streebog) g(h, n, m []uint32) {
	copy(s.data, h)
	s.xlps(s.data, n)
	copy(s.ki, s.data)
	s.xlps(s.data, m)
	for i := range 11 {
		s.xlps(s.ki, gostStreebogC[i])
		s.xlps(s.data, s.ki)
	}
	s.xlps(s.ki, gostStreebogC[11])
	streebogXor512(s.data, s.ki)
	streebogXor512(h, s.data)
	streebogXor512(h, m)
}

func (s *streebog) loadBlock(b []byte) {
	for i := range 16 {
		s.block[i] = le32(b, i*4)
	}
}

// gostDigest2012 computes the Streebog digest of data (bitLength 256 or 512).
func gostDigest2012(data []byte, bitLength int) []byte {
	s := newStreebog(bitLength)
	n := len(data)
	rem := n % 64
	q := (n - rem) / 64
	for i := range q {
		s.loadBlock(data[i*64 : i*64+64])
		s.g(s.h, s.n, s.block)
		streebogAdd512(s.n, s.buffer512)
		streebogAdd512(s.sigma, s.block)
	}
	// Final (short) block, padded with 0x01.
	b := make([]byte, 64)
	copy(b, data[q*64:q*64+rem])
	b[rem] = 0x01
	s.loadBlock(b)
	b0 := make([]uint32, 16)
	b0[0] = uint32(rem << 3)
	s.g(s.h, s.n, s.block)
	streebogAdd512(s.n, b0)
	streebogAdd512(s.sigma, s.block)
	s.g(s.h, s.buffer0, s.n)
	s.g(s.h, s.buffer0, s.sigma)

	words := s.h[8:16]
	if bitLength != 256 {
		words = s.h[0:16]
	}
	out := make([]byte, len(words)*4)
	for i, w := range words {
		putLE32(out, i*4, w)
	}
	return out
}

// ---------------------------------------------------------------------------
// GOST R 34.11-94.
// ---------------------------------------------------------------------------

// gostC2 is the IV constant for the 94 hash chaining variable C[2].
var gostC2 = []byte{
	0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF,
	0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00,
	0x00, 0xFF, 0xFF, 0x00, 0xFF, 0x00, 0x00, 0xFF,
	0xFF, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0xFF,
}

// gost94 holds the chaining state for a GOST R 34.11-94 digest.
type gost94 struct {
	cipher    *gostCipher
	c         [4][]byte
	h, m, sum []byte
}

// gostHashP is the P permutation: K[i+1+4(k-1)] = d[8i+k].
func gostHashP(d []byte) []byte {
	k := make([]byte, 32)
	for i := range 8 {
		k[4*i] = d[i]
		k[1+4*i] = d[8+i]
		k[2+4*i] = d[16+i]
		k[3+4*i] = d[24+i]
	}
	return k
}

// gostHashA is the A transform: A(x) = (x0 ^ x1) || x3 || x2 || x1. It mutates
// and returns d.
func gostHashA(d []byte) []byte {
	a := make([]byte, 8)
	for j := range 8 {
		a[j] = d[j] ^ d[j+8]
	}
	copy(d[0:24], d[8:32])
	copy(d[24:32], a)
	return d
}

// gostHashFw is the ψ (fw) transform over 16 little-endian 16-bit words.
func gostHashFw(d []byte) {
	get := func(i int) uint16 { return uint16(d[2*i]) | uint16(d[2*i+1])<<8 }
	set := func(i int, v uint16) { d[2*i] = byte(v); d[2*i+1] = byte(v >> 8) } // #nosec G115 -- splitting a 16-bit word into two bytes
	w15 := get(0) ^ get(1) ^ get(2) ^ get(3) ^ get(12) ^ get(15)
	for i := range 15 {
		set(i, get(i+1))
	}
	set(15, w15)
}

// encrypt runs one GOST 28147-89 ECB block, writing to s[sOff:sOff+8].
func (g *gost94) encrypt(key, s []byte, sOff int, d []byte, dOff int) {
	copy(s[sOff:sOff+8], g.cipher.encryptECB(key, d[dOff:dOff+8]))
}

// process is the GOST R 34.11-94 compression over a 32-byte block.
func (g *gost94) process(d []byte, dOff int) {
	s := make([]byte, 32)
	u := make([]byte, 32)
	v := make([]byte, 32)
	w := make([]byte, 32)
	copy(g.m, d[dOff:dOff+32])
	copy(u, g.h)
	copy(v, g.m)
	for j := range 32 {
		w[j] = u[j] ^ v[j]
	}
	g.encrypt(gostHashP(w), s, 0, g.h, 0)
	for i := 1; i < 4; i++ {
		tmpA := gostHashA(u)
		for j := range 32 {
			u[j] = tmpA[j] ^ g.c[i][j]
		}
		v = gostHashA(gostHashA(v))
		for j := range 32 {
			w[j] = u[j] ^ v[j]
		}
		g.encrypt(gostHashP(w), s, i*8, g.h, i*8)
	}
	for range 12 {
		gostHashFw(s)
	}
	for n := range 32 {
		s[n] ^= g.m[n]
	}
	gostHashFw(s)
	for n := range 32 {
		s[n] = g.h[n] ^ s[n]
	}
	for range 61 {
		gostHashFw(s)
	}
	copy(g.h, s)
}

// summing accumulates d into the 256-bit checksum sum.
func (g *gost94) summing(d []byte) {
	var carry int
	for i := range 32 {
		s := int(g.sum[i]) + int(d[i]) + carry
		g.sum[i] = byte(s) // #nosec G115 -- low byte of the sum; the carry is propagated below
		carry = s >> 8
	}
}

// gostDigest94 computes the GOST R 34.11-94 digest of data.
func gostDigest94(cipher *gostCipher, data []byte) []byte {
	g := &gost94{cipher: cipher, h: make([]byte, 32), m: make([]byte, 32), sum: make([]byte, 32)}
	for i := range 4 {
		g.c[i] = make([]byte, 32)
	}
	copy(g.c[2], gostC2)
	n := len(data)
	rem := n % 32
	q := (n - rem) / 32
	for i := range q {
		b := data[i*32 : i*32+32]
		g.summing(b)
		g.process(b, 0)
	}
	if rem > 0 {
		c := make([]byte, 32)
		copy(c, data[q*32:q*32+rem])
		g.summing(c)
		g.process(c, 0)
	}
	l := make([]byte, 32)
	n8, k := n*8, 0
	for n8 > 0 {
		l[k] = byte(n8 & 0xff)
		k++
		n8 /= 256
	}
	g.process(l, 0)
	g.process(g.sum, 0)
	return cloneBytes(g.h)
}

// GOSTHash computes a GOST R 34.11 hash (94 or 2012 Streebog).
type GOSTHash struct{}

// Meta returns the operation metadata.
func (GOSTHash) Meta() core.OpMeta {
	return core.OpMeta{
		Name:        "GOST Hash",
		Module:      "Hashing",
		Description: "The GOST hash function, defined in the standards GOST R 34.11-94 and GOST 34.311-95 is a 256-bit cryptographic hash function. It was initially defined in the Russian national standard GOST R 34.11-94 Information Technology – Cryptographic Information Security – Hash Function. The equivalent standard used by other member-states of the CIS is GOST 34.311-95.<br><br>This function must not be confused with a different Streebog hash function, which is defined in the new revision of the standard GOST R 34.11-2012.<br><br>The GOST hash function is based on the GOST block cipher.",
		InfoURL:     "https://wikipedia.org/wiki/GOST_(hash_function)",
		InputType:   core.TypeArrayBuffer,
		OutputType:  core.TypeString,
	}
}

// Args returns the argument definitions.
func (GOSTHash) Args() []core.ArgDef {
	return []core.ArgDef{
		{Name: "Algorithm", Type: core.ArgOption, Value: []string{"GOST 28147 (1994)", "GOST R 34.11 (Streebog, 2012)"}},
		{Name: "Digest length", Type: core.ArgOption, Value: []string{"256", "512"}},
		{Name: "sBox", Type: core.ArgOption, Value: gostSBoxNames},
	}
}

// Run computes the digest.
func (GOSTHash) Run(in *core.Dish, args []any) (*core.Dish, error) {
	algorithm := args[0].(string)
	sBox := args[2].(string)
	data := in.Bytes()

	var digest []byte
	if algorithm == "GOST 28147 (1994)" {
		cipher, err := newGostCipher(gostAlgo{
			version:    1989,
			length:     64,
			mode:       "ES",
			sBoxName:   sBox,
			block:      "ECB",
			keyMeshing: "NO",
			padding:    "NO",
		})
		if err != nil {
			return nil, err
		}
		digest = gostDigest94(cipher, data)
	} else {
		bitLength := 256
		if args[1].(string) == "512" {
			bitLength = 512
		}
		digest = gostDigest2012(data, bitLength)
	}
	return core.NewDish([]byte(hex.EncodeToString(digest)), core.TypeString), nil
}
