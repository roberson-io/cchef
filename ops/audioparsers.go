...
func mapCommonFrom(r *audioReport, source map[string]string, mapping [][2]string) {
...
func parseMp3(b []byte, r *audioReport) {
	processID3v2(b, r)
	processID3v1(b, r)
...
func processID3v2(b []byte, r *audioReport) {
	r.addSystem("id3v2")
	...
	frames := []any{}
	raw := newOMap().set("header", tag.header).set("frames", frames)
	r.raw.set("id3v2", raw)
	...
	for _, f := range tag.frames {
		...
		case f.id == "COMM":
			...
			if comm != nil {
				if text, ok := comm.vals["text"].(string); ok {
...
func processGeobFrame(f id3Frame, entry *omap, r *audioReport) {
	d := f.data
	...
	entry.set("geob", newOMap().
		set("mimeType", mimeType).
		set("filename", filename).
		set("description", description).
		set("object_bytes", objectBytes))

	n := r.countEmbedded(func(e *omap) bool { return e.vals["source"] == "id3v2:GEOB" })
	r.addEmbedded(newOMap().
	...
	if strings.Contains(mt, "c2pa") || strings.Contains(mt, "jumbf") {
		...
		embedding, _ := c2pa.vals["embedding"].([]any)
		c2pa.set("embedding", append(embedding, newOMap().
...
func processID3v1(b []byte, r *audioReport) {
...
func parseID3v1(b []byte) (*omap, map[string]string) {
	if len(b) < 128 {
		return nil, nil
	}
	off := len(b) - 128
...
func decodeTxxx(d []byte) any {
	...
	return newOMap().set("description", nullIfEmpty(desc)).set("value", nullIfEmpty(value))
}

// decodeCommFrame reads a comment frame, which names the language it is in.
func decodeCommFrame(d []byte) *omap {
	if len(d) < 5 {
		return nil
	}
	encoding := int(d[0])
...
func parseApeV2BestEffort(b []byte) *omap {
	...
	for off+8 < end && len(items) < 5000 {
		...
		off += valueSize
		items = append(items, newOMap().set("key", key).set("value", value).set("flags", itemFlags))
	}
...
func parseRiffWave(b []byte, r *audioReport, maxText int) {
	...
	for _, c := range enumerateChunks(b, 12, len(b), 50000) {
		chunkList = append(chunkList, newOMap().
...
func processRiffChunk(b []byte, c riffChunk, riff, info *omap, r *audioReport, maxText int) {
	switch {
	case c.id == "ds64":
		riff.set("ds64", newOMap().set("present", true).set("size", c.size))
		r.addSystem("bw64_ds64")
		if r.container.vals["type"] == "wav" {
...
func parseBext(b []byte, off, size int) *omap {
...
func parseFlac(b []byte, r *audioReport, maxText int) {
	r.addSystem("flac_metablocks")

	blockList := []any{}
	flac := newOMap().set("blocks", blockList)
	r.raw.set("flac", flac)

	for _, blk := range parseFlacMetaBlocks(b) {
		blockList = append(blockList, newOMap().
		...
		case "PICTURE":
			...
			n := r.countEmbedded(func(e *omap) bool {
				id, _ := e.vals["id"].(string)
...
func parseVorbisComment(buf []byte) (*omap, []vorbisComment) {
	off := 0
	...
	for i := 0; i < count && off+4 <= len(buf) && len(list) < 20000; i++ {
		...
		if eq := strings.Index(s, "="); eq > 0 {
			...
			list = append(list, newOMap().set("key", c.key).set("value", c.value))
		}
	}
	return newOMap().set("vendor", vendor).set("comments", list), comments
}

// mapVorbisCommon fills in the common tags from a comment block.
func mapVorbisCommon(r *audioReport, comments []vorbisComment) {
...
func parseOgg(b []byte, r *audioReport) {
...
func parseMp4BestEffort(b []byte, r *audioReport) {
	...
	for off+8 <= len(b) && len(atoms) < 2000 {
		...
		atoms = append(atoms, newOMap().set("type", typ).set("size", size).set("offset", off))
		seen[typ] = true
		off += size
	}
...
func parseAiffBestEffort(b []byte, r *audioReport, maxText int) {
	...
	for off+8 <= len(b) && len(index) < 2000 {
		...
		size := u32be(b, off+4)
		dataOff := off + 8
		index = append(index, newOMap().set("id", id).set("size", size).set("offset", off))

		if aiffTextChunks[id] {
			text := safeUtf8(sliceRange(b, dataOff, dataOff+min(size, maxText)))
			textChunks = append(textChunks, newOMap().
...
func parseAacAdts(b []byte, r *audioReport) {
...
func parseAc3(b []byte, r *audioReport) {
	r.addSystem("ac3_bsi")
	if len(b) < 8 {
		return
	}
...
func parseWmaAsf(b []byte, r *audioReport) {
	...
	for i := 0; i < numObjects && off+24 <= headerEnd; i++ {
		...
		guid4 := b[off : off+4]
		objects = append(objects, newOMap().
...
func readAsfObject(b []byte, r *audioReport, ensureASF func() *omap, off, objSize int) {
	guid4 := b[off : off+4]
	dataOff := off + 24
	dataLen := objSize - 24

	switch {
	case equalBytes(guid4, asfContentDescriptionGUID) && dataLen >= 10:
	...
	case equalBytes(guid4, asfExtendedContentGUID) && dataLen >= 2:
...
func parseAsfContentDescription(b []byte, off int) (*omap, map[string]string) {
