...
// not-found / wrong-type guard branches that are awkward to reach via jsParse.
func TestJSParserHelpers(t *testing.T) {
	noType := jsObject{{k: "name", v: "x"}}
	if got := jsNodeType(noType); got != "" {
		t.Errorf("jsNodeType(no type field) = %q, want empty", got)
	}
	if got := jsNodeType("not-an-object"); got != "" {
		t.Errorf("jsNodeType(non-object) = %q, want empty", got)
	}
	...
	obj := jsObject{{k: "type", v: "X"}}
	if got := jsFieldSlice(obj, "missing"); got != nil {
		t.Errorf("jsFieldSlice(missing) = %v, want nil", got)
	}
	if got := jsField(obj, "missing"); got != nil {
		t.Errorf("jsField(missing) = %v, want nil", got)
	}
	if got := jsField("not-an-object", "type"); got != nil {
		t.Errorf("jsField(non-object) = %v, want nil", got)
	}
	if jsIsPropertyKey("not-a-node", "x") {
		t.Error("jsIsPropertyKey(non-node) = true, want false")
	}
	litKey := jsObject{{k: "type", v: "Literal"}, {k: "value", v: 1.0}}
	if jsIsPropertyKey(litKey, "constructor") {
		t.Error("jsIsPropertyKey(numeric Literal) = true, want false")
	}
	if got := jsIdentName(obj); got != "" {
		t.Errorf("jsIdentName(no name) = %q, want empty", got)
	}
	if jsFieldIsNil(obj, "missing") {
		t.Error("jsFieldIsNil(missing) = true, want false")
	}
	if jsFieldBool(obj, "missing") {
		t.Error("jsFieldBool(missing) = true, want false")
	}
...
// unreachable through jsParse; it is exercised directly with a crafted node.
func TestJSFinishGroupArrowYield(t *testing.T) {
	...
	if len(ph.params) != 1 || jsIdentName(ph.params[0]) != "yield" {
...
func TestJSParserAST(t *testing.T) {
	...
	for sc.Scan() {
		...
		if got := jsStringify(ast, 2); got != vec.Want {
