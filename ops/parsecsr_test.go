...
const (
	csrRSA1024In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrRSA2048In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrEC256In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrEC384In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrEC521In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrDSA1024In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrDSA2048In = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrSANIn = `-----BEGIN CERTIFICATE REQUEST-----
...
Requested Extensions
  ...
  Subject Alternative Name:
	...
	csrKeyUsageIn = `-----BEGIN CERTIFICATE REQUEST-----
MIIDJDCCAgwCAQAwcjELMAkGA1UEBhMCQ0gxDzANBgNVBAgMBlp1cmljaDEPMA0G
A1UEBwwGWnVyaWNoMRMwEQYDVQQKDApFeGFtcGxlIFJFMRYwFAYDVQQLDA1JVCBE
ZXBhcnRtZW50MRQwEgYDVQQDDAtleGFtcGxlLmNvbTCCASIwDQYJKoZIhvcNAQEB
...
Requested Extensions
  ...
  Subject Alternative Name:
    DNS: example.com
    DNS: www.example.com`
	csrExtKeyUsageIn = `-----BEGIN CERTIFICATE REQUEST-----
...
func TestParseCSR(t *testing.T) {
	one := func(name, in, want string) opCase {
		return opCase{name, in, want, core.Recipe{{Op: "Parse CSR", Args: []any{"PEM"}}}}
	}
	runCases(t, []opCase{
		one("RSA 1024", csrRSA1024In, csrRSA1024Out),
		one("RSA 2048", csrRSA2048In, csrRSA2048Out),
		one("EC P-256", csrEC256In, csrEC256Out),
		one("EC P-384", csrEC384In, csrEC384Out),
		one("EC P-521", csrEC521In, csrEC521Out),
		one("DSA 1024", csrDSA1024In, csrDSA1024Out),
		one("DSA 2048", csrDSA2048In, csrDSA2048Out),
		one("various SAN types", csrSANIn, csrSANOut),
		one("various key usages", csrKeyUsageIn, csrKeyUsageOut),
		one("various extended key usages", csrExtKeyUsageIn, csrExtKeyUsageOut),
	})
}

// TestParseCSRErrors covers the empty-input and malformed-input paths.
func TestParseCSRErrors(t *testing.T) {
	if got, err := runOp(t, "Parse CSR", "", "PEM"); err != nil || got != "No input" {
		t.Fatalf("No input: got %q err %v", got, err)
	}
	for _, c := range []struct{ name, input string }{
		{"bad PEM", "not a pem"},
		{"truncated DER", "-----BEGIN CERTIFICATE REQUEST-----\nMAMCAQE=\n-----END CERTIFICATE REQUEST-----"},
	} {
		t.Run(c.name, func(t *testing.T) {
			if _, err := runOp(t, "Parse CSR", c.input, "PEM"); err == nil {
...
// Crafted DER building blocks for direct helper tests.
const (
	oidRSA        = "2a864886f70d010101"
	oidECPub      = "2a8648ce3d0201"
	oidP256       = "2a8648ce3d030107"
	oidDSA        = "2a8648ce380401"
	oidServerAuth = "2b06010505070301"
)

func spki(alg, bitContent string) string {
	return derTLV("30", alg+derTLV("03", "00"+bitContent))
}

func TestFormatCSRPublicKeyKinds(t *testing.T) {
	...
	dsaParams := derTLV("30", derTLV("02", "00cc")+derTLV("02", "00dd")+derTLV("02", "00ee"))
	dsa := spki(derTLV("30", derTLV("06", oidDSA)+dsaParams), derTLV("02", "00ff"))
	unknown := spki(derTLV("30", derTLV("06", "2a0304")+"0500"), "00")

	for _, c := range []struct{ name, hexIn, want string }{
		{"RSA", rsa, "Algorithm:      RSA"},
		{"EC", ec, "Algorithm:      ECDSA"},
		{"DSA", dsa, "Algorithm:      DSA"},
		{"unknown", unknown, "unsupported public key algorithm"},
	} {
		t.Run(c.name, func(t *testing.T) {
			got, err := formatCSRPublicKey(c.hexIn, 0)
			if err != nil || !contains(got, c.want) {
...
func TestFormatCSRSignatureKinds(t *testing.T) {
	dsaSig := derTLV("30", derTLV("02", "01")+derTLV("02", "02"))
	for _, c := range []struct{ alg, sig, want string }{
		{"SHA256withDSA", dsaSig, "R:"},
		{"SHA256withRSA", "abcd", "Signature:      ab:cd"},
		{"SHA256withECDSA", "30ff", "Signature:      30:ff"},
	} {
		got, err := formatCSRSignature(c.alg, c.sig)
		if err != nil || !contains(got, c.want) {
			t.Fatalf("%s: got %q err %v", c.alg, got, err)
		}
	}
}

func TestCSRDescribers(t *testing.T) {
	// basicConstraints: CA=true with pathLen.
	bc := derTLV("30", "0101ff"+derTLV("02", "03"))
	if got := describeBasicConstraints(bc); got[0] != "CA = true" || got[1] != "PathLenConstraint = 3" {
		t.Fatalf("basicConstraints: %v", got)
	}
	// keyUsage: empty bits => "(none)".
	if got := describeKeyUsage(derTLV("03", "0700")); got[0] != "(none)" {
		t.Fatalf("keyUsage none: %v", got)
	}
	// extKeyUsage: known OID, and empty => "(none)".
	eku, _ := describeExtendedKeyUsage(derTLV("30", derTLV("06", oidServerAuth)))
	if eku[0] != "TLS Web Server Authentication" {
		t.Fatalf("eku: %v", eku)
	}
	if none, _ := describeExtendedKeyUsage(derTLV("30", "")); none[0] != "(none)" {
		t.Fatalf("eku none: %v", none)
	}
	// subjectAltName: DIR (directoryName).
	san, _ := describeSubjectAltName(derTLV("30", derTLV("a4", derName("x"))))
	if san[0] != "DIR: /CN=x" {
		t.Fatalf("san dir: %v", san)
	}
}

func TestCSRSmallHelpers(t *testing.T) {
	if ensureHexPositive("abc") != "0abc" || ensureHexPositive("80") != "0080" || ensureHexPositive("7f") != "7f" {
		t.Fatal("ensureHexPositive")
	}
	if bitLenOfHex("zz") != 0 || trimBigHex("zz") != "" {
		t.Fatal("bad-hex helpers should return zero values")
	}
	if _, _, _, ok := ecCurveInfo("1.2.3"); ok {
		t.Fatal("ecCurveInfo unknown curve should be !ok")
	}
	if _, err := formatCSR(bad); err == nil {
		t.Fatal("formatCSR should error on malformed input")
	}
	if _, err := csrExtensionList("30ff", 0); err == nil {
		t.Fatal("csrExtensionList should error on malformed attrs")
	}
}

func contains(s, sub string) bool { return strings.Contains(s, sub) }

// mkCSRHex assembles a CertificationRequest from certificationRequestInfo
// children, with the given signatureAlgorithm and empty signature.
func mkCSRHex(sigAlg string, criKids ...string) string {
	return derTLV("30", derTLV("30", strings.Join(criKids, ""))+sigAlg+derTLV("03", "00"))
}

func TestParseCSRDepthGuards(t *testing.T) {
	rsaAlg := derTLV("30", derTLV("06", "2a864886f70d01010b")+"0500")
	dsaAlg := derTLV("30", derTLV("06", "608648016503040302")+"0500")
	badAlg := derTLV("30", "")
	vInt := derTLV("02", "00")
	dn := derName("x")
	...
	badAttrs := derTLV("a0", derTLV("30", derTLV("06", "2a864886f70d01090e")+derTLV("31", "300a")))

	for _, c := range []struct{ name, hex string }{
		{"cri<3", mkCSRHex(rsaAlg, vInt, dn)},
		{"bad subject", mkCSRHex(rsaAlg, vInt, badDN, rsa)},
		{"bad spki", mkCSRHex(rsaAlg, vInt, dn, badSPKI)},
		{"bad root sigalg", mkCSRHex(badAlg, vInt, dn, rsa)},
		{"bad dsa signature", mkCSRHex(dsaAlg, vInt, dn, rsa)},
		{"bad attrs", mkCSRHex(rsaAlg, vInt, dn, rsa, badAttrs)},
	} {
		t.Run(c.name, func(t *testing.T) {
			if _, err := formatCSR(c.hex); err == nil {
...
func TestCSRExtensionSlots(t *testing.T) {
	// An extensionRequest carrying an unknown extension lands in the extra slot.
	unknownExt := derTLV("30", derTLV("06", "2a0304")+derTLV("04", "00"))
	extReq := derTLV("30", derTLV("06", "2a864886f70d01090e")+derTLV("31", derTLV("30", unknownExt)))
	attrs := derTLV("a0", extReq)
	got, err := formatRequestedExtensions(attrs, 0)
	if err != nil || !contains(got, "1.2.3.4:") {
		t.Fatalf("unknown ext: %q err %v", got, err)
	}
	// No attributes and a non-extensionRequest attribute both yield no extensions.
	if l, _ := csrExtensionList("00", -1); l != nil {
		t.Fatal("nil attrs")
	}
	other := derTLV("a0", derTLV("30", derTLV("06", "2a0304")+derTLV("31", "")))
	if l, _ := csrExtensionList(other, 0); l != nil {
		t.Fatal("non-extensionRequest attr")
	}
	// DSA signature with a malformed value errors.
	if _, err := formatCSRSignature("SHA256withDSA", "300a"); err == nil {
		t.Fatal("bad dsa sig")
	}
	// describe* error and behavioral paths.
	if _, err := describeExtendedKeyUsage(bad); err == nil {
		t.Fatal("eku err")
	}
	if eku, _ := describeExtendedKeyUsage(derTLV("30", derTLV("06", "2a0304"))); eku[0] != "1.2.3.4" {
		t.Fatalf("unknown eku: %v", eku)
	}
	if _, err := describeSubjectAltName(bad); err == nil {
		t.Fatal("san err")
	}
	_ = describeBasicConstraints(bad)
}

func TestFormatCSRPublicKeyGuards(t *testing.T) {
	if _, err := formatCSRPublicKey(derTLV("30", derTLV("30", "")), 0); err == nil {
		t.Error("spki<2")
	}
	// RSA with a malformed public-key bit string.
	rsaBad := spki(derTLV("30", derTLV("06", oidRSA)+"0500"), "300a")
	if _, err := formatCSRPublicKey(rsaBad, 0); err == nil {
		t.Error("rsa bad bitstring")
	}
	// EC with an unknown named curve.
	ecBad := spki(derTLV("30", derTLV("06", oidECPub)+derTLV("06", "2a0304")), "0400")
	if _, err := formatCSRPublicKey(ecBad, 0); err == nil {
		t.Error("unknown curve")
	}
	// DSA with too few parameters.
	dsaBad := spki(derTLV("30", derTLV("06", oidDSA)+derTLV("30", derTLV("02", "00cc"))), derTLV("02", "00ff"))
	if _, err := formatCSRPublicKey(dsaBad, 0); err == nil {
		t.Error("dsa params<3")
	}
	// A requested extension with a single child errors; a describe* sub-error propagates.
	badExtReq := derTLV("a0", derTLV("30", derTLV("06", "2a864886f70d01090e")+
		derTLV("31", derTLV("30", derTLV("30", derTLV("06", "551d0e"))))))
	if _, err := formatRequestedExtensions(badExtReq, 0); err == nil {
		t.Error("ext<2")
	}
	badEKU := derTLV("a0", derTLV("30", derTLV("06", "2a864886f70d01090e")+
		derTLV("31", derTLV("30", derTLV("30", derTLV("06", "551d25")+derTLV("04", "300a"))))))
	if _, err := formatRequestedExtensions(badEKU, 0); err == nil {
		t.Error("eku sub-error")
	}
	badSAN := derTLV("a0", derTLV("30", derTLV("06", "2a864886f70d01090e")+
		derTLV("31", derTLV("30", derTLV("30", derTLV("06", "551d11")+derTLV("04", "300a"))))))
	if _, err := formatRequestedExtensions(badSAN, 0); err == nil {
		t.Error("san sub-error")
	}
	// An attribute with a single child is skipped (continue).
	badAttr := derTLV("a0", derTLV("30", derTLV("06", "2a864886f70d01090e")))
	if l, _ := csrExtensionList(badAttr, 0); l != nil {
		t.Errorf("attr<2 should be skipped: %v", l)
	}
}
