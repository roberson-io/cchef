...
func parseFunctionalColour(s string) (d3Colour, bool) {
	...
	alpha := 1.0
	if len(parts) == 4 {
		alpha = jsParseFloat(strings.TrimSpace(parts[3]))
	}
	switch name {
	case "rgb", "rgba":
		return d3Colour{
	...
	case "hsl", "hsla":
		h := jsParseFloat(strings.TrimSpace(parts[0]))
...
func colourComponent(s string, full float64) float64 {
	...
	if pct, ok := strings.CutSuffix(s, "%"); ok {
		return jsParseFloat(pct) / 100 * full
	}
...
func formatRGB(c d3Colour) string {
	channels := jsNum(clampChannel(c.r)) + ", " + jsNum(clampChannel(c.g)) + ", " + jsNum(clampChannel(c.b))
	alpha := 1.0
	...
	return "rgba(" + channels + ", " + jsNum(alpha) + ")"
}
