...
var cpNames = func() []string {
	names := make([]string, len(cpCharsets))
	for i, c := range cpCharsets {
		names[i] = c.name
	}
	return names
}()
...
var cpByName = func() map[string]int {
	m := make(map[string]int, len(cpCharsets))
...
func (DecodeText) Run(in *core.Dish, args []any) (*core.Dish, error) {
	...
	out, err := cptableDecode(cp, in.Bytes())
	if err != nil {
		return nil, err
	}
...
func (EncodeText) Run(in *core.Dish, args []any) (*core.Dish, error) {
	...
	out, err := cptableEncode(cp, in.String())
	if err != nil {
		return nil, err
	}
...
func (TextEncodingBruteForce) Run(in *core.Dish, args []any) (*core.Dish, error) {
	...
	obj := make(jsObject, 0, len(cpCharsets))
	for _, c := range cpCharsets {
		var val string
		if decode {
			if s, err := cptableDecode(c.cp, in.Bytes()); err != nil {
				val = "Could not decode."
			} else {
				val = s
			}
		} else {
			if b, err := cptableEncode(c.cp, in.String()); err != nil {
				val = "Could not decode."
			} else {
				val = mimeByteArrayToUtf8(b)
			}
		}
		obj = append(obj, jsPair{k: c.name, v: val})
	}
	return core.NewDish([]byte(jsStringify(obj, 4)), core.TypeJSON), nil
}
