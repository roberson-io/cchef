...
// parentheses on). It walks the ESTree AST produced by jsParse and emits
...
// jsGenerate renders an ESTree AST to formatted source using CyberChef's options.
func jsGenerate(ast jsObject, indent, quotes string, semicolons bool) string {
	return jsGenerateComments(ast, indent, quotes, semicolons, nil)
}

// jsGenerateComments is jsGenerate with comment emission driven by the attached
// comment set (nil to disable).
func jsGenerateComments(ast jsObject, indent, quotes string, semicolons bool, cs *jsCommentSet) string {
...
func isIdentPartES5(c rune) bool {
	return c == '$' || c == '_' ||
		...
		(c >= 0x80 && jsIsIdentifierPart(int(c)))
}
...
func isJSWhiteSpace(cp int) bool { return jsIsWhiteSpace(cp) }
...
func (g *jsGen) generateIdentifier(node any) string { return jsIdentName(node) }

func (g *jsGen) generateAsyncPrefix(node any, spaceRequired bool) string {
	if jsFieldBool(node, "async") {
		if spaceRequired {
			return "async "
		}
...
func (g *jsGen) generateMethodPrefix(prop any) string {
	fn := jsField(prop, "value")
	prefix := ""
	if jsFieldBool(fn, "async") {
		prefix += g.generateAsyncPrefix(fn, !jsFieldBool(prop, "computed"))
	}
	if jsFieldBool(fn, "generator") && g.generateStarSuffix(fn) != "" {
		prefix += "*"
	}
	return prefix
}
...
func (g *jsGen) generateFunctionParams(node any) string {
	params := jsFieldSlice(node, "params")
	typ := jsNodeType(node)
	if typ == "ArrowFunctionExpression" && len(params) == 1 && jsNodeType(params[0]) == "Identifier" {
...
func (g *jsGen) generateFunctionBody(node any) string {
	...
	if jsNodeType(node) == "ArrowFunctionExpression" {
		result += g.space + "=>"
	}
	if jsFieldBool(node, "expression") {
		result += g.space
		expr := g.generateExpression(jsField(node, "body"), pAssignment, eTTT)
		if len(expr) > 0 && expr[0] == '{' {
			expr = "(" + expr + ")"
		}
		result += expr
	} else {
		result += g.maybeBlock(jsField(node, "body"), sTTFF)
	}
	return result
}
...
func (g *jsGen) maybeBlockSuffix(stmt any, result string) string {
	...
	if jsNodeType(stmt) == "BlockStatement" && len(g.jsNodeLeading(stmt)) == 0 && !ends {
...
func (g *jsGen) generateIterationForStatement(operator string, stmt any, flags int) string {
	result := "for" + g.space + "("
	g.withIndent(func() {
		left := jsField(stmt, "left")
		if jsNodeType(left) == "VariableDeclaration" {
			g.withIndent(func() {
				kind, _ := jsField(left, "kind").(string)
				decls := jsFieldSlice(left, "declarations")
		...
		result = g.join(result, g.generateExpression(jsField(stmt, "right"), pAssignment, eTTT)) + ")"
	})
	result += g.maybeBlock(jsField(stmt, "body"), flags)
	return result
}
