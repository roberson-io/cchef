...
//
// The two formats share a package because a value is often encoded in one and
// referenced from the other. Both directions work on the ordered JSON model in
// [github.com/roberson-io/cchef/internal/jsonval], so an object's keys come
// back out in the order they went in — which decides the bytes, and is what
// makes the encoding reproducible.
//
// AMF3's reference tables mean a string, object or trait appears once and is
// pointed at afterwards; [NewTables3] holds that state for one encoding or
// decoding pass.
package amfcodec
...
// object — with objects held as jsonval.Object so the order the input gave survives
...
// Writer builds AMF output. A value AMF3's 29-bit length cannot express is
...
// Err reports a value the format could not express. It is recorded here rather
// than returned from every call that writes one, so a long encoding is not a
// chain of error checks.
func (w *Writer) Err() error { return w.err }

func (w *Writer) byte(b byte)    { w.buf = append(w.buf, b) }
func (w *Writer) bytes(b []byte) { w.buf = append(w.buf, b...) }
func (w *Writer) uint16(n int)   { w.buf = binary.BigEndian.AppendUint16(w.buf, uint16(n)) } // #nosec G115 -- callers bound n to amf0MaxShortString
func (w *Writer) uint32(n int)   { w.buf = binary.BigEndian.AppendUint32(w.buf, uint32(n)) } // #nosec G115 -- callers bound n to a slice length
...
// ParseJSON reads JSON into the value kinds the encoders handle, keeping
...
func amf0EncodeString(w *Writer, s string, withMarker bool) {
...
// Decode0 reads one AMF0 value. refs holds the objects seen so far, which
...
func amf0DecodeScalar(r *Reader, marker byte) (v any, handled bool, err error) {
	switch marker {
	case amf0Number:
		d, err := r.double()
		return d, true, err
...
func amf0DecodeComposite(r *Reader, refs *[]any, marker byte) (any, error) {
	switch marker {
	case amf0Object:
...
func amf0ReadStrictArray(r *Reader, refs *[]any) (any, error) {
	n, err := r.uint32()
	if err != nil {
		return nil, err
	}
...
func amf0ReadString(r *Reader, long bool) (string, error) {
	var n int
	var err error
	if long {
		n, err = r.uint32()
...
func amf0ReadObject(r *Reader, refs *[]any) (any, error) {
	obj := jsonval.Object{}
...
// NewTables3 returns the reference tables one AMF3 encoding or decoding pass
// keeps: a value written once is pointed at by index afterwards.
func NewTables3() *amf3Tables { return &amf3Tables{} }

// Encode3 writes one value in AMF3. Nothing is written by reference: the
...
func amf3EncodeString(w *Writer, s string) {
	w.u29(len(s)<<1 | 1)
	w.bytes([]byte(s))
}
...
func amf3EncodeObject(w *Writer, obj jsonval.Object) error {
	w.byte(amf3Object)
...
func amf3DecodeScalar(r *Reader, t *amf3Tables, marker byte) (v any, handled bool, err error) {
	switch marker {
...
func amf3ReadDate(r *Reader, t *amf3Tables) (any, error) {
	ref, err := r.u29()
	if err != nil {
		return nil, err
	}
...
func amf3ReadByteArray(r *Reader, t *amf3Tables) (any, error) {
	ref, err := r.u29()
	if err != nil {
		return nil, err
	}
...
func amf3ReadString(r *Reader, t *amf3Tables) (string, error) {
	n, err := r.u29()
	if err != nil {
		return "", err
	}
...
// keys.
func amf3ReadArray(r *Reader, t *amf3Tables) (any, error) {
	n, err := r.u29()
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		assoc = append(assoc, jsonval.Pair{K: key, V: v})
	}
	...
	if len(assoc) > 0 {
		...
		for i, e := range dense {
			assoc = append(assoc, jsonval.Pair{K: fmt.Sprint(i), V: e})
		}
		t.objects[at] = assoc
		return assoc, nil
	}
...
func amf3ReadObject(r *Reader, t *amf3Tables) (any, error) {
	n, err := r.u29()
	if err != nil {
		return nil, err
	}
	...
	for _, m := range members {
		...
		obj = append(obj, jsonval.Pair{K: m, V: v})
	}
	if dynamic {
		for {
			key, err := amf3ReadString(r, t)
...
func amf3ReadTraits(r *Reader, t *amf3Tables, n int) (members []string, dynamic bool, err error) {
