...
//
// [Parse] walks the JPEG markers to the EXIF segment and decodes its TIFF
// structure into a [TagStore], which preserves the order tags were found in
// and keeps the first value seen for a repeated tag. The parser matches the
// npm exif-parser library, including its use of exceptions for control flow,
// mirrored here as a scoped panic/recover that [Parse] converts to an error.
package exif
...
func newExifTagStore() *TagStore { return &TagStore{vals: map[string]any{}} }

func (s *TagStore) setIfAbsent(name string, v any) {
...
func (s *TagStore) set(name string, v any) { s.vals[name] = v }
...
func castDegreeValues(store *TagStore) {
...
func castDateValues(store *TagStore) {
...
func parseDateTimeParts(dateParts, timeParts []string) (float64, bool) {
	...
	year, ok1 := jsnum.ParseInt(dateParts[0], 10)
	month, ok2 := jsnum.ParseInt(dateParts[1], 10)
	day, ok3 := jsnum.ParseInt(dateParts[2], 10)
	hh, ok4 := jsnum.ParseInt(timeParts[0], 10)
	mm, ok5 := jsnum.ParseInt(timeParts[1], 10)
	ss, ok6 := jsnum.ParseInt(timeParts[2], 10)
...
func parseDateWithTimezoneFormat(s string) (float64, bool) {
	...
	tzHours, _ := jsnum.ParseInt(tzParts[0], 10)
	tzMins := 0
	if len(tzParts) > 1 {
		tzMins, _ = jsnum.ParseInt(tzParts[1], 10)
	}
...
// ValueString renders a simplified tag value the way JS string interpolation
