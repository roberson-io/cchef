...
// mustHex decodes a hex literal for tests that build TIFF structures byte by
// byte.
...
func TestExifValueString(t *testing.T) {
	...
	if ValueString(float64(1.5)) != "1.5" {
		t.Error("float")
	}
	if ValueString([]any{float64(1), float64(2), float64(3)}) != "1,2,3" {
		t.Error("array join")
	}
	if ValueString(nil) != "undefined" {
		t.Error("nil")
	}
	if ValueString(rational{1, 2}) != "" {
		t.Error("default")
	}
}

func TestJSNumberString(t *testing.T) {
	if jsnum.String(math.NaN()) != "NaN" ||
		jsnum.String(math.Inf(1)) != "Infinity" ||
		jsnum.String(math.Inf(-1)) != "-Infinity" ||
		jsnum.String(2) != "2" {
