...
// mustHex decodes a hex literal for the tests that build buffers byte by byte.
...
func carveOne(t *testing.T, buf []byte, ext string) (string, []byte) {
	t.Helper()
	for _, f := range filesig.Scan(buf, nil) {
...
func TestExtractFileWithoutCarver(t *testing.T) {
	sig := filesig.Sig{Name: "Test", Extension: "xyz", MIME: "application/x-test"}
...
func TestCarveAbortsCleanly(t *testing.T) {
	...
	for _, f := range filesig.Scan(buf, []string{"Images"}) {
...
func TestMP3FrameSizeRejects(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if _, ok := mp3FrameSize(bytestream.New(tc.bytes)); ok {
...
func TestMP3Layers(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got, ok := mp3FrameSize(bytestream.New(tc.header))
			if !ok {
...
func TestByteStreamAdvance(t *testing.T) {
	...
	back := bytestream.New([]byte{1, 2, 3})
...
// TestCanCarve covers the lookup callers use to ask whether a signature's named
// algorithm exists, without having to hold a candidate buffer to try it on.
func TestCanCarve(t *testing.T) {
	if !CanCarve("PNG") {
		t.Error("PNG has a carver but CanCarve says otherwise")
	}
	if CanCarve("NoSuchFormat") {
		t.Error("an unknown name reported as carvable")
	}
	if CanCarve("") {
		t.Error("the empty name, which means no carver, reported as carvable")
	}
	// Every name the signature table gives must resolve, or a file type is
	// advertised as recoverable and then cannot be recovered.
	for _, cat := range filesig.Signatures {
		for _, ft := range cat.Types {
			if ft.Carver != "" && !CanCarve(ft.Carver) {
				t.Errorf("%s names carver %q, which does not exist", ft.Name, ft.Carver)
			}
		}
	}
}
