// Package codepage decodes and encodes the legacy character sets CyberChef
// supports, reproducing the codepage npm library it uses.
//
// [Charsets] lists every character set in CyberChef's own order, each with the
// codepage number [Decode] and [Encode] take. The tables are compressed and
// embedded rather than generated at build time, since together they are large.
//
// Reproducing codepage means reproducing where it departs from the standards:
// its US-ASCII maps every byte to the code point of the same value, and its
// UTF-8 does not replace an invalid sequence but reads it as the start of a
// longer one.
package codepage
...
// cpKind maps a codepage number to its dispatch kind (see Charsets).
var cpKind = func() map[int]string {
	m := make(map[int]string, len(Charsets))
...
// Decode decodes bytes in the given codepage to a string, reproducing
...
// Encode encodes a string into the given codepage, reproducing
