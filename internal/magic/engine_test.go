...
func TestMagicText(t *testing.T) {
	if got := opsutil.BytesAsText([]byte("héllo")); got != "héllo" {
		t.Errorf("valid UTF-8 changed: %q", got)
	}
	got := opsutil.BytesAsText([]byte{0xff, 0xd8, 0xff})
...
func TestMagicLanguageName(t *testing.T) {
	...
	if got := LanguageName("zzz"); got != "zzz" {
...
func TestMagicScore(t *testing.T) {
	base := Option{LangScores: []LangScore{{Score: 1000}}, Entropy: 0}
	...
	good := Option{LangScores: []LangScore{{Score: 10}}, FileType: &filesig.Sig{}, Useful: true}
...
func TestMagicRankPrefersRecipesOverBareSuggestions(t *testing.T) {
	bare := Option{
		LangScores:  []LangScore{{Score: 1}},
		MatchingOps: []magicCheck{{Op: "From Hex"}},
	}
	ran := Option{
		LangScores: []LangScore{{Score: 900}},
		Recipe:     core.Recipe{{Op: "From Hex"}},
	}
	ranked := magicRank([]Option{bare, ran})
	...
	ranked = magicRank([]Option{ran, bare})
	if len(ranked[0].Recipe) == 0 {
...
func TestMagicPrune(t *testing.T) {
	nothing := []LangScore{{Probability: 0}}
	cases := []struct {
		name string
		opt  Option
		keep bool
	}{
		{"empty and not useful", Option{Data: "", LangScores: nothing}, false},
		{"empty but useful", Option{Data: "", LangScores: nothing, Useful: true, IsUTF8: true}, true},
		{"nothing found at all", Option{Data: "x", LangScores: nothing}, false},
		{"a language was found", Option{Data: "x", LangScores: []LangScore{{Probability: 0.5}}}, true},
		{"a file was found", Option{Data: "x", LangScores: nothing, FileType: &filesig.Sig{}}, true},
		{"the data is text", Option{Data: "x", LangScores: nothing, IsUTF8: true}, true},
		{"an operation matched", Option{Data: "x", LangScores: nothing, MatchingOps: []magicCheck{{}}}, true},
		{"the crib matched", Option{Data: "x", LangScores: nothing, MatchesCrib: true}, true},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := len(magicPrune([]Option{c.opt})) == 1
			if got != c.keep {
...
func TestMagicRunRecipeAbandonsFailures(t *testing.T) {
	run := &Runner{Registry: testRegistry()}
...
func TestMagicEncodingGuessesNeedsTheOperation(t *testing.T) {
	run := &Runner{Registry: core.NewRegistry()}
...
// testRegistry is a registry holding just upperOp, so the engine tests do not
// depend on the operation catalog: what matters here is how the Runner treats
// an operation, not which ones exist.
func testRegistry() *core.Registry {
	reg := core.NewRegistry()
	reg.Register(upperOp{})
	return reg
}

// upperOp is a minimal working operation registered as "To Upper case", the
// name the engine's checks table refers to.
type upperOp struct{}

func (upperOp) Meta() core.OpMeta {
	return core.OpMeta{Name: "To Upper case", InputType: core.TypeString, OutputType: core.TypeString}
}

func (upperOp) Args() []core.ArgDef {
	return []core.ArgDef{{Name: "Scope", Type: core.ArgOption, Value: []string{"All"}}}
}

func (upperOp) Run(in *core.Dish, args []any) (*core.Dish, error) {
	return core.NewDish([]byte(strings.ToUpper(in.String())), core.TypeString), nil
}
