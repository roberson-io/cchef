...
//
// A [Runner] looks at data, asks which operations claim to be able to decode
// something of that shape, runs them from the registry it is given, and looks
// at what comes out — recursively, to a given depth — then ranks the recipes
// it found by how much like ordinary text or a known file the result became.
// Ported from CyberChef's lib/Magic.mjs; the Magic operation renders what
// [Runner.Speculate] returns.
package magic
...
// BruteForceLen is how much of the data brute forcing works on, since
...
// Runner carries the settings for one invocation, so the recursion does not
...
func magicChiSquared(observed, expected [256]float64) (score, probability float64) {
	...
	return score, 1 - opsutil.ChiSquaredCDF(score, float64(len(observed)-1))
}

// magicDetectLanguage ranks the languages by how well the data's byte
// frequencies fit each, best first.
func magicDetectLanguage(data []byte, extensive bool) []LangScore {
	if len(data) == 0 {
		return []LangScore{{Lang: "Unknown", Score: math.MaxFloat64, Probability: 0}}
	}
	...
	for lang, expected := range langs {
		...
		scores = append(scores, LangScore{Lang: lang, Score: score, Probability: probability})
	}
...
func magicDetectFileType(data []byte) *filesig.Sig {
	types := filesig.Detect(data, nil)
	if len(types) == 0 {
		return nil
	}
	return &types[0]
...
func magicPatternMatches(pattern string, data []byte) bool {
	...
	return re.MatchString(opsutil.BytesAsText(data))
}
...
func magicIsMime(prefix string, data []byte) bool {
	for _, t := range filesig.Detect(data, nil) {
...
func magicSnippet(data []byte) string {
	runes := []rune(opsutil.BytesAsText(data))
...
func (m *Runner) describe(data []byte, recipe core.Recipe, useful bool) Option {
	...
	return Option{
		...
		Useful:      useful,
		MatchesCrib: m.Crib != nil && m.Crib.MatchString(opsutil.BytesAsText(data)),
	}
}
...
func (m *Runner) runRecipe(recipe core.Recipe, data []byte) []byte {
...
// check promised.
func (m *Runner) attempt(data []byte, check magicCheck, prevOp string) ([]byte, bool) {
...
func (m *Runner) Speculate(data []byte, depth int, recipe core.Recipe, useful bool) []Option {
	...
	options := []Option{m.describe(data, recipe, useful)}
...
func magicPrune(options []Option) []Option {
	kept := options[:0]
...
func magicRank(options []Option) []Option {
...
func magicScore(o Option) float64 {
...
func (m *Runner) bruteForce(data []byte) []magicGuess {
	sample := data
	if len(sample) > BruteForceLen {
		sample = sample[:BruteForceLen]
	}
...
func (m *Runner) encodingGuesses(sample []byte) []magicGuess {
