...
// parseXML reproduces @xmldom/xmldom's DOMParser.parseFromString(source) called
...
func parseXML(src string) *xmlNode {
	p := &xmlParser{
		src:      normalizeLineEndings(src),
		doc:      &xmlNode{typ: xmlDocument},
...
type xmlParser struct {
	...
	pos      int
	doc      *xmlNode
	stack    []*xmlNode     // currently open elements, innermost last
	haveRoot bool           // the documentElement has been seen
	limbo    *xmlNode       // sink for content after the root closes (discarded)
...
func (p *xmlParser) parent() *xmlNode {
	if len(p.stack) > 0 {
		return p.stack[len(p.stack)-1]
	}
	if p.haveRoot {
		if p.limbo == nil {
			p.limbo = &xmlNode{typ: xmlDocument}
		}
		return p.limbo
	}
	return p.doc
...
func (p *xmlParser) append(n *xmlNode) {
	par := p.parent()
	n.parent = par
...
func (p *xmlParser) parseText() {
	...
	p.append(&xmlNode{typ: xmlText, data: decodeEntities(p.src[start:p.pos])})
}
...
func (p *xmlParser) parseMarkup() {
	...
	default:
		...
		p.pos++
		if len(p.stack) > 0 {
			p.append(&xmlNode{typ: xmlText, data: "<"})
		}
	}
}
...
func (p *xmlParser) parseComment() {
	...
	if end < 0 {
		p.append(&xmlNode{typ: xmlComment, data: p.src[p.pos:]})
		p.pos = len(p.src)
		return
	}
	p.append(&xmlNode{typ: xmlComment, data: p.src[p.pos : p.pos+end]})
	p.pos += end + len("-->")
}
...
func (p *xmlParser) parseCData() {
	...
	if end < 0 {
		p.append(&xmlNode{typ: xmlCData, data: p.src[p.pos:]})
		p.pos = len(p.src)
		return
	}
	p.append(&xmlNode{typ: xmlCData, data: p.src[p.pos : p.pos+end]})
	p.pos += end + len("]]>")
}
...
func (p *xmlParser) parsePI() {
	...
	p.append(&xmlNode{typ: xmlPI, name: target, data: strings.TrimLeft(data, " \t\n")})
}
...
func (p *xmlParser) parseStartTag() {
	p.pos++ // consume '<'
	name := p.readName()
	el := &xmlNode{typ: xmlElement, name: name}
...
func (p *xmlParser) parseAttributes(el *xmlNode) {
	for p.pos < len(p.src) {
		p.skipSpace()
		if p.pos >= len(p.src) {
			return
		...
		el.attrs = append(el.attrs, xmlAttr{name: name, value: value})
	}
}
