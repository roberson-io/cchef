package ops

import "hash"

// MD2 (RFC 1319). Ported from crypto-api's md2 hasher.

var md2SBox = [256]byte{
	,
}

// md2Rounds is the fixed round count (crypto-api's default).
const md2Rounds = 18

type md2 struct {
	x        [16]byte
	nx       int
	hash     [48]byte
	checksum [16]byte
	rounds   int
}

func newMD2() hash.Hash { return newMD2Rounds(md2Rounds) }

// newMD2Rounds builds MD2 with a configurable round count (the standalone MD2
// operation exposes this; the default is 18).
func newMD2Rounds(rounds int) hash.Hash { return &md2{rounds: rounds} }

func (d *md2) Reset()         { *d = md2{rounds: d.rounds} }
func (d *md2) Size() int      { return 16 }
func (d *md2) BlockSize() int { return 16 }

func (d *md2) Write(p []byte) (int, error) {
	n := len(p)
	for len(p) > 0 {
		c := copy(d.x[d.nx:], p)
		d.nx += c
		p = p[c:]
		if d.nx == 16 {
			d.block(d.x[:])
			d.nx = 0
		}
	}
	return n, nil
}

func (d *md2) Sum(in []byte) []byte {
	e := *d
	// PKCS7 pad to the next 16-byte boundary.
	pad := byte(16 - e.nx) // #nosec G115 -- 16-e.nx is in [1,16], a valid byte
	var p [16]byte
	for i := range p {
		p[i] = pad
	}
	_, _ = e.Write(p[:pad])
	// Append the checksum and process it as a final block.
	cs := e.checksum
	_, _ = e.Write(cs[:])
	return append(in, e.hash[:16]...)
}

func (d *md2) block(block []byte) {
	for i := range 16 {
		d.hash[16+i] = block[i]
		d.hash[32+i] = block[i] ^ d.hash[i]
	}
	t := byte(0)
	for i := 0; i < d.rounds; i++ {
		for j := range 48 {
			d.hash[j] ^= md2SBox[t]
			t = d.hash[j]
		}
		t += byte(i)
	}
	t = d.checksum[15]
	for i := range 16 {
		d.checksum[i] ^= md2SBox[block[i]^t]
		t = d.checksum[i]
	}
}
