...
// produced by jsParseFull, and computes which node each comment leads or trails.
//
// esprima/estraverse mutate the AST in place (node.leadingComments/
// trailingComments). Here the AST nodes are immutable jsObject values, so instead
...
func jsNodeKey(node any) string {
	obj, _ := node.(jsObject)
	r, _ := jsField(obj, "range").([]any)
	if len(r) != 2 {
		return ""
	}
	return fmt.Sprintf("%s@%d:%d", jsNodeType(obj), jsRangeVal(r[0]), jsRangeVal(r[1]))
}
...
func jsUpperBound(items []jsObject, pred func(jsObject) bool) int {
	lo, hi := 0, len(items)
	for lo < hi {
		mid := (lo + hi) / 2
		if pred(items[mid]) {
...
func extendCommentRange(c *jsComment, tokens []jsObject) {
	c.extLo, c.extHi = c.lo, c.hi
	target := jsUpperBound(tokens, func(t jsObject) bool {
		r, _ := jsField(t, "range").([]any)
		return jsRangeVal(r[0]) > c.lo
	})
	if target != len(tokens) {
		r, _ := jsField(tokens[target], "range").([]any)
		c.extHi = jsRangeVal(r[0])
	}
	if target-1 >= 0 {
		r, _ := jsField(tokens[target-1], "range").([]any)
...
type jsTraverser struct {
	enter, leave func(jsObject) int
}

func (t *jsTraverser) visit(node jsObject) int {
	if t.enter != nil {
		switch t.enter(node) {
		case jsTravBreak:
			return jsTravBreak
	...
	for _, key := range jsVisitorKeys[jsNodeType(node)] {
		switch child := jsField(node, key).(type) {
		case jsObject:
...
func (t *jsTraverser) leaveOnly(node jsObject) int {
...
// jsAttachComments computes leading/trailing comment attachments for the AST,
// transliterating estraverse.attachComments (the non-preserve-blank-lines path).
func jsAttachComments(tree jsObject, rawComments, tokens []jsObject) *jsCommentSet {
	...
	for i, rc := range rawComments {
		r, _ := jsField(rc, "range").([]any)
		comments[i] = &jsComment{
			typ:   jsStr(jsField(rc, "type")),
			value: jsStr(jsField(rc, "value")),
			lo:    jsRangeVal(r[0]),
			hi:    jsRangeVal(r[1]),
		}
	}
...
// the node begins.
func (a *jsCommentAttacher) enterLeading(node jsObject) int {
...
// where the node ends.
func (a *jsCommentAttacher) leaveTrailing(node jsObject) int {
