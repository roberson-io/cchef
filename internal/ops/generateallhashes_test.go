...
func genAllHashesRun(t *testing.T, input string, r core.Recipe) string {
	t.Helper()
...
func TestGenerateAllHashes(t *testing.T) {
	const full = `MD2:          dd34716876364a02d0195e2fb9ae2d1b
...
CTPH:         A:E:E
`
	got := genAllHashesRun(t, "test", core.Recipe{{Op: "Generate all hashes", Args: []any{"All", true}}})
	if got != full {
		t.Fatalf("full mismatch:\n got:\n%s\nwant:\n%s", got, full)
	}
}

// Length filter keeps only digests of the matching bit-length.
func TestGenerateAllHashesLength(t *testing.T) {
	const len128 = `MD2:          dd34716876364a02d0195e2fb9ae2d1b
MD4:          db346d691d7acc4dc2625db19f9e3f52
MD5:          098f6bcd4621d373cade4e832627b4f6
RIPEMD-128:   f1abb5083c9ff8a9dbbca9cd2b11fead
BLAKE2b-128:  44a8995dd50b6657a037a7839304535b
...
NT Hash:      0CB6948805F797BF2A82807973B89537
`
	got := genAllHashesRun(t, "test", core.Recipe{{Op: "Generate all hashes", Args: []any{"128", true}}})
	if got != len128 {
		t.Fatalf("len128 mismatch:\n got:\n%s\nwant:\n%s", got, len128)
	}
}

// Without names, only the raw digests of the matching length are emitted.
func TestGenerateAllHashesNoNames(t *testing.T) {
	const len256 = `93c8a7d0ff132f325138a82b2baa98c12a7c9ac982feb6c5b310a1ca713615bd
...
`
	got := genAllHashesRun(t, "test", core.Recipe{{Op: "Generate all hashes", Args: []any{"256", false}}})
	if got != len256 {
		t.Fatalf("len256 mismatch:\n got:\n%s\nwant:\n%s", got, len256)
	}
}

// A failing sub-operation propagates its error. The static table only names real
// ops, so this swaps in a bogus entry to exercise the error path directly.
func TestGenerateAllHashesError(t *testing.T) {
	saved := genAllHashesList
	genAllHashesList = []genHashEntry{{"X", "No Such Op", false, nil}}
	defer func() { genAllHashesList = saved }()

	_, err := runOp(t, "Generate all hashes", "test", "All", true)
	if err == nil {
		t.Fatal("expected error from unknown sub-operation")
	}
}
