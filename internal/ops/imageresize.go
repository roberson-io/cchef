...
// resizeStrategyNames maps cchef's option labels to Jimp's ResizeStrategy keys.
var resizeStrategyNames = map[string]string{
...
// jimpResize resizes img to wf×hf (rounded, min 1) using the named strategy.
func jimpResize(img *image.NRGBA, wf, hf float64, mode string) *image.NRGBA {
	w := int(math.Round(wf))
	if w == 0 {
		w = 1
	}
...
// jimpScale scales img by factor f.
func jimpScale(img *image.NRGBA, f float64, mode string) *image.NRGBA {
	return jimpResize(img, float64(img.Rect.Dx())*f, float64(img.Rect.Dy())*f, mode)
}

// jimpScaleToFit scales img to fit within wf×hf, preserving aspect ratio.
func jimpScaleToFit(img *image.NRGBA, wf, hf float64, mode string) *image.NRGBA {
...
// jimpCropExact extracts the w×h region at (x,y). Returns an error if the region
// falls outside the source (Jimp throws a RangeError, which CyberChef surfaces).
func jimpCropExact(img *image.NRGBA, x, y, w, h int) (*image.NRGBA, error) {
	W := img.Rect.Dx()
	src := img.Pix
...
// jimpContain scales img to fit within w×h and centres it (per alignment) on a
// transparent canvas. Ported from @jimp/plugin-contain.
func jimpContain(img *image.NRGBA, w, h, alignH, alignV int, mode string) *image.NRGBA {
	W, H := float64(img.Rect.Dx()), float64(img.Rect.Dy())
	var f float64
	...
	c := jimpScale(img, f, mode)
	canvas := image.NewNRGBA(image.Rect(0, 0, w, h))
	bx := round((float64(w-c.Rect.Dx()) / 2) * float64(alignH))
	by := round((float64(h-c.Rect.Dy()) / 2) * float64(alignV))
	jimpBlit(canvas, c, bx, by)
	return canvas
}

// jimpCover scales img to fill w×h and crops the aligned region. Ported from
// @jimp/plugin-cover.
func jimpCover(img *image.NRGBA, w, h, alignH, alignV int, mode string) (*image.NRGBA, error) {
	W, H := float64(img.Rect.Dx()), float64(img.Rect.Dy())
	var f float64
	if float64(w)/float64(h) > W/H {
		f = float64(w) / W
