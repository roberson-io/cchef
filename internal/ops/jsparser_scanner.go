...
func jsIsWhiteSpace(cp int) bool {
	switch cp {
...
// jsIsIdentifierStart / jsIsIdentifierPart mirror esprima's Character helpers:
...
func jsIsIdentifierPart(cp int) bool {
	return cp == 0x24 || cp == 0x5F ||
...
type jsScanner struct {
	...
	trackComment bool         // when set, comments are collected (jsParseFull)
	comments     []jsObject   // collected comments (type/value/range)
...
func (s *jsScanner) recordComment(multiLine bool, vStart, vEnd, rStart, rEnd int) {
	...
	s.comments = append(s.comments, jsObject{
		{k: "type", v: typ},
...
func (s *jsScanner) scanIdentifierEscape(isStart bool) rune {
	...
	valid := jsIsIdentifierPart(int(ch))
	if isStart {
