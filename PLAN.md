...
The core engine, recipe/URL machinery, CLI, docs, and a **curated set of 472
...
- **472 operations** (`internal/ops/`), each a faithful port with tests
...
40k LOC) implementing the ~472 registered operations. Nothing about it is broken;
...
than one category.
Currently **469 unique** CyberChef operations are covered (468 directly plus
`SHA2`, exposed as the `sha224`, `sha256`, `sha384` and `sha512` subcommands),
which is where the 472 cchef subcommands come from.
